...
# ********************
    ...
    def __init__(self,
        ...
        self.wait_for_load_timer=None
        self.load_delay_timer=None
        self.wait_for_end_timer=None
...
# ***************************
    ...
    def load_first_track(self):
        ...
        if self.medialist.start() is False:
            ...
            self.remove_list_empty=GLib.timeout_add(5000,self.remove_list_empty_message)
        else:
    ...
    def remove_list_empty_message(self):
        if self.remove_list_empty!=None:
            GLib.source_remove(self.remove_list_empty)
            self.remove_list_empty=None
    ...
    def wait_for_load(self):
        if self.wait_for_load_timer!=None:
            GLib.source_remove(self.wait_for_load_timer)
            self.wait_for_load_timer=None        
        ...
        if self.next_player is not None:
            if self.next_player.get_play_state() == 'load-failed':
                ...
                if self.previous_player is  None  and self.next_player.get_play_state() == 'loaded':
                    ...
                    self.wait_for_load_timer=GLib.timeout_add(10,self.what_next)
                else:
                    self.wait_for_load_timer=GLib.timeout_add(50,self.wait_for_load)
        else:
            self.wait_for_load_timer=GLib.timeout_add(200,self.wait_for_load)           
    ...
    def show_next_track(self):
        ...
        self.load_delay_timer=GLib.timeout_add(self.load_delay,self.what_to_load_next)
    ...
    def what_to_load_next(self):
        if self.load_delay_timer!=None:
            GLib.source_remove(self.load_delay_timer)
            self.load_delay_timer=None
...
##        self.previous_player=None    # safer to delete the player here rather than in player as play-state is read elsewhere.
    ...
    def wait_for_end(self):
        if self.wait_for_end_timer!=None:
            GLib.source_remove(self.wait_for_end_timer)
            self.wait_for_end_timer=None
        self.mon.trace(self, self.pretty_state())
        ok_to_end=0
        ...
        if self.next_player is None or self.next_player.get_play_state() in ('initialised','unloaded','load-failed'):
            ...
            ok_to_end+=1
        if ok_to_end != 2:
            self.wait_for_end_timer=GLib.timeout_add(50,self.wait_for_end)
        else:
...
# ***************************
    ...
    def stop_timers(self):
        pass
            ...
            #self.canvas.xafter_cancel(self.duration_timer)
