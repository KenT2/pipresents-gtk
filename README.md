PI PRESENTS  - Version 1.6.1b (PP-GTK)
==================================

FEBRUARY 2026: THE FUTURE OF PP-GTK
-----------------------------------

The original intention in 2023 was that PP-GTK should replace PP-KMS as the basis for forward development.
 This intention has changed and this version will not be developed, PP-KMS will. PP-GTK will be maintained
  and it is hoped that the fields in the profile of the yet to be developed PP-TKX will stay as close to PP-GTK as possible.

This change of plan is because:
 - Pi Presents KMS can now run on Trixie and hence RPi5 under X11.
 - There is a limitation in GTK4/Wayland that makes playing videos with text/image backgrounds impossible.
 - Review of the likelihood of future development of key components. Three years on from 2023 it is now clear that
      * Tkinter and PIL (now Pillow) are still being developed.
      * X11 will continue to be available via the XWayland compatibility layer.



INTRODUCTION
------------
pipresents-gtk is a major rewrite of PP to make it compatible with Raspberry Pi Model 5.
...
In addition other changes have been made to remove unsupported software. I have also tidied up some of the profile fields that have grown like topsy over the 10 years of Pi Presents development.
 ...
 . mplayer based audio player replaced by MPV video player playing audio tracks

GTK runs on all models of RPi under X11
...
-------------
	...
	* must use the latest version of 64 bit RPi OS Trixie with Desktop, (not the Lite version)
...
Install 64 bit RPi OS Trixie
---------------------------     
Image a SD Card with the RPi OS Trixie with desktop (64 Bit).
...
THE GUI WAY
        ...
        There should now be a file 'pipresents-gtk-master.zip' in your /downloads directory.
            Unzip the file and copy the directory to /home. Rename it to pipresents
...
You will see an error message which is because you have no profiles.
...
NEW - Videos can only be displayed 'full-windowed':
        - If the -f option is used then 'full-windowed' is the size of the display and the video is centred on the display
        - For the 'develop window' the video is displayed covering the window and centred on the window.
        The show canvas and MPV video window fields of a profile are ignored
        MPV aspect mode is honoured.
...
The -b command line option to control blanking is removed. Raspberry Pi Control Centre now controls blanking
