...
class OSCMonitor(object):
    ...
    def __init__(self):
        ...
        if self.osc_config.slave_enabled !='yes':
            self.mon.log(self,'EROR: OSC Slave is not enabled in pp_oscmonitor.cfg')
            exit()
    ...
    def run_app(self):
        ...
        if self.osc_config.slave_enabled !='yes':
            self.mon.log(self,'Error: Slave not enabled in oscmonitor.cfg')
            return

        if self.osc_config.this_unit_ip =='':
            self.mon.log(self,'Error: IP of own unit must be provided in oscmonitor.cfg')
            return

        if self.osc_config.listen_port =='':
            self.mon.log(self,'Error: Listen port must be provided in oscmonitor.cfg')
            return
