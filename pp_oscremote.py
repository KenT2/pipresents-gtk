...
class OSCRemote(object):
    ...
    def __init__(self):
        ...
        if self.osc_config.this_unit_ip =='':
            self.mon.log(self,'Error: IP of own unit must be provided in oscremote.cfg')
    ...
    def run_app(self):
        ...
        if self.osc_config.master_enabled !='yes':
            self.mon.log(self,'Error: OSC Master is not enabled in oscremote.cfg')
            return

        if self.osc_config.reply_listen_port =='':
            self.mon.log(self,'Error: Reply Listen port is not set in oscremote.cfg')
            return
...
# ***************************************
    ...
    def send_message(self):
        if self.slave_unit =='/':
            self.mon.log(self,'Error: slave unit OSC name not set')
            return
        if self.osc_config.slave_units_ip=='':
            self.mon.log(self,'Error: slave unit IP not set')
            return
    ...
    def sendto(self,client,dest, address,arg_list):
        ...
        try:
            ...
            self.mon.log(self,'Error: error in OSC client when sending OSC command: '+ str(e))
    ...
    def open_existing_profile(self):
        ...
        if os.path.exists(initial_dir) is False:
            self.mon.log(self,"Error: Profiles directory not found: " + initial_dir + "\n\nHint: Data Home option must end in pp_home")
            return
    ...
    def open_profile(self,dir_path):
        ...
        if os.path.exists(showlist_file) is False:
            self.mon.log(self,"Error: Not a Profile: " + dir_path + "\n\nHint: Have you opened the profile directory?")
            return
    ...
    def open_showlist(self,profile_dir):
        ...
        if os.path.exists(showlist_file) is False:
            self.mon.log(self,"Error: showlist file not found at " + profile_dir + "\n\nHint: Have you opened the profile directory?")
            self.app_exit()
